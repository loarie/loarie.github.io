...
function albersUsaPr() {
  ...
  // XXX?
	var americanSamoa = d3.geo.conicEqualArea()
      .rotate([170, 0])
      .center([0, -14])
      .parallels([-10, -20]);
  ...
  albersUsa.invert = function(coordinates) {
    ...
    return (y >= .120 && y < .234 && x >= -.425 && x < -.214 ? alaska
        ...
        : y >= .204 && y < .234 && x >= .320 && x < .380 ? puertoRico
				: y >= .204 && y < .234 && x >= .320 && x < .380 ? guam
				: y >= .204 && y < .234 && x >= .320 && x < .380 ? americanSamoa
  ...
  albersUsa.stream = function(stream) {
    var lower48Stream = lower48.stream(stream),
        ...
        puertoRicoStream = puertoRico.stream(stream),
				...
				americanSamoaStream = americanSamoa.stream(stream);
    return {
      point: function(x, y) {
  ...
  albersUsa.translate = function(_) {
    ...
    guamPoint = guam
        .translate([x - .220 * k, y + .140 * k])
        .clipExtent([[x - .214 * k + ε, y + .145 * k + ε], [x - .190 * k - ε, y + .166 * k - ε]])
        .stream(pointStream).point;

    americanSamoaPoint = americanSamoa
        .translate([x - .155 * k, y + .147 * k])
        .clipExtent([[x - .190 * k + ε, y + .145 * k + ε], [x - .130 * k - ε, y + .166 * k - ε]])
        .stream(pointStream).point;
